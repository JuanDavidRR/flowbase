...
const SELECTED_PLAN_STORAGE_KEY = "flowbase:selectedPlan";
...
export default function DemoRequestForm() {
  ...
  // Picks up the plan chosen on the pricing section: sessionStorage covers
  // this island mounting after the click (client:visible), the custom event
  // covers it already being mounted when a different plan is picked.
  useEffect(() => {
    ...
    function handlePlanSelected(event: Event) {
      const planName = (event as CustomEvent<{ planName: string }>).detail
        ?.planName;
      if (planName) setSelectedPlan(planName);
    }

    window.addEventListener("flowbase:plan-selected", handlePlanSelected);
    return () =>
      window.removeEventListener("flowbase:plan-selected", handlePlanSelected);
  }, []);
  ...
  function handleSubmit(event: SubmitEvent<HTMLFormElement>) {
    ...
    console.info(
      "[Flowbase demo] Form submitted (placeholder, no backend wired up):",
      values,
    );
    ...
    return (
      ...
      >
        <h3
          ...
          tabIndex={-1}
          className="font-heading text-lg font-semibold focus:outline-none"
        >
    ...
    >
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Select
          ...
          onValueChange={(value) =>
            updateField("companySize", (value as string) ?? "")
          }
        >
          <SelectTrigger
            ...
            aria-describedby={
              errors.companySize ? `${formId}-company-size-error` : undefined
            }
          >
